          ...
          <table id="example1" class="table table-bordered table-striped">
            ...
            <?php
                 // var_dump($secretariats);
                    if(isset($secretariats['secretariat']) and count($secretariats['secretariat'])>0)
                    {
                        $i=0;
                        foreach($secretariats['secretariat'] as $liste)
                        {
                            ?>
                            ...
                            echo '<td id="bp">' . $liste->nombre. '       <button type="button" class="btn btn-info" data-toggle="modal" data-target="#myModal">Motifs</button></td>';
                    ...
                    {
                        echo '<tr><td colspan="6"  class="h3 text-center"><a href="#" class="text-warning">Aucun Secretariat pour le moment ...</a><td></tr>';
                    }
                    ?>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Motifs de supension</h4>
      </div>
      ...
      <table id="example2" class="table table-bordered table-striped">
            ...
            <?php
                  // var_dump($secretariats);
                    if(isset($secretariats['motif']) and count($secretariats['motif'])>0)
                    {
                        $i=0;
                        foreach($secretariats['motif'] as $liste)
                        {
                            ?>

                            <?php
                            echo '<tr id="' . $liste->id_motif . '"><td class="text-center">' . ++$i . '</td>';
                            echo '<td id="nomsecretariat">' . $liste->designation. '</td>';
                            echo '<td id="nom">' . $liste->date. '</td>';
                    ...
                    {
                        echo '<tr><td colspan="6"  class="h3 text-center"><a href="#" class="text-warning">Aucun motifs pour le moment ...</a><td></tr>';
                    }
                    ?>
...
function(){
  ...
  var form_data = {
    ...
    $.ajax({
        url: "<?php echo site_url('admin/Secretariat/active_compte'); ?>",
        type: 'POST',
        async : false,
        data: form_data,
    ...
    ajax : '1'
        ...
        $.ajax({
        url: "<?php echo site_url('admin/Secretariat/updateinfo'); ?>",
        type: 'POST',
        async : false,
        data: form_data,
        success: function(msg) {
