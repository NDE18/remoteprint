...
class Parametre_model extends My_model
{
    ...
    public function get_localite()
    {
        ...
        $resultat=$this->db->select('r.id as identifiant,COUNT(s.ville) as nombre , r.*,v.*,COUNT(op.id) as idappeloffre,COUNT(op.user) as nbuser,COUNT(o.id) as offre')
                  ->from($this->region.' r')
                  ->join($this->ville.' v','v.region=r.id')
                  ->join($this->secretariat.' s','s.ville=v.id','left')
                  ->join($this->appelOffre.' op','op.ville=v.id','left')
                  ->join($this->offre.' o','o.secretariat=s.id','left')
                  ->group_by('v.region')
                  ->get()->result();
