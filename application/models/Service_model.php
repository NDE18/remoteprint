...
// ce modele permet de recuperé tous les informations sur un utilisateurs (les secretariats)
    ...
    public function Valider_service($data)
    {
    $service = $this->db->select('*')
      ...
      //  var_dump($transaction);
        ...
        foreach ($service as $value) {
            $caracteristique = $this->db->select('*')
      ->from($this->caracteristique)
      ->where(array('id'=>$data['id']))
      ->get()->result();
      if($caracteristique==null)
      {
        $resultat= array('val'=>0,'msg'=>'desole vous ne pouvez pas valider un service qui n a aucune caractéristique');
        echo json_encode($resultat);
      }
        $modification_service= array('statut' => 1,'priorite'=>$data['priorite'],'dateValidation'=>date('Y-m-d h:i:sa'));
        $this->db->where('id',$data['id']);
        $this->db->update('service',$modification_service);
        $notification=array("receiver_id"=>0,"message"=>"Salut à vous chere utilisateurs nous ténons à vous informer que le service de ".$value->nom,"vu"=>0);
        $this->db->insert($this->notification, $notification);
        $resultat= array('val'=>1,'msg'=>'le service a bien été validée');
          echo json_encode($resultat);
      }
    }
      return !empty($resultat)?$resultat:null;
    ...
    public function caracteristique($data=array())
    {
        ...
        if($service_existe!=null)
       {
        $resultat=array('val'=>0,'msg'=>'desole mais la caracteristique de se service existe deja');
       }
       else{
